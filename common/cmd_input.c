...
#if defined(CONFIG_EMBEDSKY_INPUT)
#include <tq_input.h>
...
static void lcd_choice_menu_usage(u8 page)
{
	...
	else
	{
		...
		Lcd_printf(0,17*6,RGB(0,0,0xff),RGB(0xc0,0xc0,0xc0),0,  "[K3:] HDMI 1280x720\" screen.      ");
		Lcd_printf(0,17*7,RGB(0,0,0xff),RGB(0xc0,0xc0,0xc0),0,	"[K4:] HDMI 1920x1080\" screen.     ");
		Lcd_printf(0,17*8,RGB(0,0,0xff),RGB(0xc0,0xc0,0xc0),0,  "[K5:] Page Up.                     ");

		printf("[K1:] W35\" screen.\r\n");
		printf("[K2:] VGA640480\" screen.\r\n");
		printf("[K3:] HDMI 1280x720\" screen..\r\n");
		printf("[K4:] HDMI 1920x1080\" screen.\r\n");
		printf("[K5:] Page UP.\r\n");

	}
	printf("[K6:] Return Parameter Menu.\r\n");
	printf("Press a key on the TQ210 Board.\r\n");
...
static void sd_download_usage(void)
{
	...
	printf("[K2:] Download Kernel or NK to Nand.\r\n");
	...
	Lcd_printf(0,17*4,RGB(0,0,0xff),RGB(0xc0,0xc0,0xc0),0,	"[K1:] Download bootloader to Nand.     ");
	Lcd_printf(0,17*5,RGB(0,0,0xff),RGB(0xc0,0xc0,0xc0),0,	"[K2:] Download Kernel or NK to Nand.   ");
...
//	embedsky_paint_table(LCD_XSIZE_TFT-26*8-4,4,26*8,16*6,0xff,NULL);
	...
	Lcd_printf(0,16*3,RGB(0,0,0xff),0,0,"======TQ210 userkey download mode======");
		...
		switch (get_button_value())
		{
			...
			case (KEY2):
			{
				...
				if(flg == 0){
					...
					sprintf(cmd_buf, "setenv lcdtype X1280Y720;");
				}	
				run_command(cmd_buf, 0);
				run_command("saveenv;", 0);
				break;
			...
			case (KEY3):
			{
				...
				if(flg == 0){
					...
					sprintf(cmd_buf, "setenv lcdtype X1920Y1080;");
				}	
				run_command(cmd_buf, 0);
				run_command("saveenv;", 0);
				break;
...
static void sd_download_shell(void)
{
	...
	if (FAT_Init())
	{
		...
		embedsky_paint_table(LCD_XSIZE_TFT-26*8-4,4,0,16*6,0xff,"Fat init failed!");
		input_exit = 1;
	}
...
//				printf("filesize=0x%x, %d\n", filesize, filesize);
				...
				{
					strcpy(cmd_buf, "relocateNK 0x41000000 $(filesize); SWince");
				}
#endif /*CONFIG_SUPPORT_WINCE*/
				else
				{
#ifdef CONFIG_MTD_DEVICE
					strcpy(cmd_buf, "nand erase kernel; nand write.jffs2 0xc0008000 kernel $(filesize)");
#else
					strcpy(cmd_buf, "nand erase 0x300000 0x400000; nand write.jffs2 0xc0008000 0x300000 $(filesize)");
#endif /* CONFIG_MTD_DEVICE */
				}
				run_command(cmd_buf, 0);
				break;
